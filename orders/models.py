...
class OrderEvent(models.Model):
    ...
    message_key = models.CharField(
        max_length=300,
        blank=True,
        help_text="Gettext message key for i18n (English template with %(name)s params)",
    )
    params = models.JSONField(
        default=dict,
        blank=True,
        help_text="Dynamic params for message_key interpolation",
    )
    msg_class = models.CharField(
        max_length=30,
        blank=True,
        help_text="CSS class for terminal log styling",
    )
